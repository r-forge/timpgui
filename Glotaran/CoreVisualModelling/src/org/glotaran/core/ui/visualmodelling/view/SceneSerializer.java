...
import org.glotaran.core.models.gta.Layout;
import org.glotaran.core.models.gta.Model;
...
public class SceneSerializer {
    ...
    public static void serialize (GraphSceneImpl scene, File file) {        
        GtaProjectScheme test = new GtaProjectScheme();
...
//        setAttribute (document, sceneElement, SCENE_EDGE_COUNTER_ATTR, Long.toString (scene.edgeIDcounter));
        ...
        for (Object node : scene.getNodes ()) {
            if (node instanceof TgmDataNode) {
               TgmDataNode node2 = (TgmDataNode) node;
               node2.getObject().getTgm();
            Widget widget = scene.findWidget (node);
            Point location = widget.getPreferredLocation ();
            Model testModel = new Model();
            Layout testLayout = new Layout();
            testLayout.setXposition(location.getX());
            testLayout.setYposition(location.getY());
            testLayout.setHeight(widget.getBounds().height);
            testLayout.setWidth(widget.getBounds().width);
            testModel.setLayout(testLayout);
            test.getModels().add(testModel);
            }
...
//            sceneElement.appendChild (edgeElement);
        ...
        test.setName("test");
        try {
            javax.xml.bind.JAXBContext jaxbCtx = javax.xml.bind.JAXBContext.newInstance(test.getClass().getPackage().getName());
